...
import {prepareAllowanceFixtures} from './prepare-allowance-fixtures.mjs';
import {prepareShadowFixtures} from './prepare-shadow-fixtures.mjs';
import {unitTestSelection} from './unit-test-selection.mjs';
const root=path.resolve(path.dirname(fileURLToPath(import.meta.url)),'..');
const scripts=JSON.parse(await fs.readFile(path.join(root,'package.json'))).scripts;
const files=await unitTestSelection(root,scripts['test:unit']);
const fixtures=await prepareTestFixtures();
const allowance=await prepareAllowanceFixtures();
const shadow=await prepareShadowFixtures();
const result=spawnSync(process.execPath,['--test','--test-reporter=tap',...files],{cwd:root,env:{...process.env,...fixtures.environment,...allowance.environment,...shadow.environment},encoding:'utf8',maxBuffer:16*1024**2});
