...
function Controls({session, sceneRef, catalog, onSelectCase}) {
  ...
  return <div className="controls">
    ...
    <label>Process<select id="mode" aria-label="Process" value={state.mode} onChange={e => dispatch({type: 'process', mode: e.target.value})}>
...
function ActionBar({session}) {
  ...
  const example=scene.workflow||spec,baseline=['greedy_geometric_baseline','staged_greedy_geometric_baseline'].includes(example.example_meta.kind);
  const exampleLabel=baseline?'Play greedy baseline':'Play teaching sequence';
  const exampleNote=baseline
    ? scene.workflow?'Mill-turn baseline: greedily remove with Turning first, then Milling; shorter reach and action ID break ties. It is not an optimal machining plan.':'Greedy geometric baseline: each step maximizes new removal, with shorter reach and action ID as tie-breaks. It is not an optimal machining plan.'
  ...
  return <>
    <div className="action-row">
      ...
      <button id="replay-example" type="button" onClick={() => dispatch({type: 'replay-start', actions: example.example})}>{exampleLabel}</button>
    </div>
...
function Metrics({session, data}) {
  const {state, live, playback, displayGym, observation, evaluation, isPreview, scene} = session;
  ...
  const reasons = {
    ...
    step_budget_reached: 'Action budget reached. Reset to start another sequence.',
  };
  let status=reasons[observation.terminal_reason];
  if(scene.workflow&&!observation.terminated&&!observation.process_can_remove_more)status='No further removal in '+(displayGym.mode==='turning'?'Turning':'Milling')+'. Switch process to continue.';
  ...
  return <>
    <div className="metrics">
      ...
      <div><span>Applied actions</span><strong id="step-value">{observation.step_count}{scene.workflow?'':' / '+data.maximumSteps}</strong></div>
    </div>
    <p id="status" role="status" aria-live="polite">{state.replay ? 'Playback · '+(displayGym.mode==='turning'?'Turning':'Milling')+' · step ' + state.replay.index + ' of ' + state.replay.actions.length : status}</p>
    <p id="scene-note">{note}</p>
  </>;
}

function SequenceHistory({history,workflow}) {
  return <div className="history-area">
    <div><h3>Your sequence</h3><ol id="history">{history.length ? history.map((row, i) => <li key={i}><span>{i + 1}. {workflow&&<b className="process-badge">{row.process==='turning'?'Turning':'Milling'}</b>} {row.label} · {row.length} mm</span><strong>{fmt(row.removed)} mm³</strong></li>) : <li className="empty">Apply an action to begin your sequence.</li>}</ol></div>
    <div><h3>What to try</h3><p>Start with the block and a short reach. Apply the same action again: it removes no additional material. Increase the reach, then try another direction. On the overhang, compare approaching from above with approaching from the right.</p><p>Industrial and uploaded mill-turn cases start in Turning. Switch freely between Turning and Milling: both processes act on the same remaining stock. The example baseline turns first, then mills the non-axisymmetric remainder.</p></div>
  </div>;
}
...
export function GymApp({data, figureBaseUrl = 'figures/', catalog, onSelectCase, initialMode, focusOnMount}) {
  ...
  useLayoutEffect(() => {
    ...
    const api = {
      snapshot: () => ({scene: scene.id, mode: state.mode, observation: live.gym.observation(), material: Array.from(live.gym.live),
        history: live.history.map(x => ({...x})), replay: state.replay ? {index: state.replay.index, length: state.replay.actions.length, process:playback?.gym.mode||state.mode} : null,
  ...
  return <section id="gym" ref={gymRef} aria-label="Interactive shadow gym" data-framework="react">
    ...
    <nav className="industrial-shortcuts" aria-label="Industrial cases"><strong>Industrial cases</strong>
      {catalog.cases.filter(s => s.industrial).map(s => <a key={s.id} href={'?case=' + s.id + '&process=turning#gym'} onClick={e => { e.preventDefault(); launch(s.id, 'turning'); }}>{s.title.split(' · ').at(-1)}</a>)}
