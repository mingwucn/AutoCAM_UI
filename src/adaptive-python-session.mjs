...
export class AdaptivePythonSession {
  ...
  async supportsTransitionRecord(){
    try{
      ...
      if(['adaptive-browser-transition-record-1','adaptive-journal-browser-transition-record-1','adaptive-prepared-browser-transition-record-1','adaptive-mixed-learning-browser-transition-record-1'].includes(JSON.parse(raw)?.schema))return true;
      throw Error('Unsupported transition record schema.');
    }catch(error){if(['Unknown browser operation','Unsupported combined browser operation','Unsupported indexed browser operation','Unsupported drill browser operation','Unsupported full mill-turn operation'].includes(error.message))return false;throw error;}
  }
