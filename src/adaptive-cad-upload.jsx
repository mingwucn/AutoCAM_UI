...
export function AdaptiveCadUpload({configuration,runtimeConfiguration,onPrepared,onInvalidate,showStockPreview=true}){
  ...
  async function prepare(event){
    ...
    try{
      const output=await prepareCadFile(file,{...configuration,stockOptions:{mode,margin,axis,depth},profile,signal:controller.signal,onProgress:text=>{if(ticket.current===id)setPhase(text);}});
