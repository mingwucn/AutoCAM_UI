...
export function StepUpload({enabled,onPrepared,onClear,hasLocalData}) {
  const workerRef=useRef(null),jobRef=useRef(0),[file,setFile]=useState(null),[process,setProcess]=useState('milling');
  ...
  async function submit(event){
    ...
    if(process==='turning'&&!(holdingLength>=0))return setError('Holding length cannot be negative.');
    ...
    try{
      ...
      worker.postMessage({type:'prepare',bytes,options:{name:file.name,process,pitch:+pitch,allowance:+allowance,lengths,axis,held,holdingLength:+holdingLength,maximumCells:300000}},[bytes]);
  ...
  return <details className="step-upload" open={running||!!error}>
    ...
    <div className="step-upload-body">
      ...
      <form onSubmit={submit}>
        ...
        <label>Process<select id="step-process" value={process} onChange={e=>setProcess(e.target.value)}><option value="milling">Milling</option><option value="turning">Turning</option></select></label>
