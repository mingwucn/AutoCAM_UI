...
export function DataSource({bundledData,renderGym,allowStepUpload=false}) {
  ...
  if(localData){const localCatalog={schema:'shadow-gym-case-catalog-1',cases:localData.scenes.map(scene=>({id:scene.id,title:scene.title,industrial:false,processes:Object.keys(scene.modes)}))},localMode=Object.keys(localData.scenes[0].modes)[0];return <>{uploader}<DatasetBoundary key={localData.scenes[0].title+'/'+localMode} retry={()=>setLocalData(null)}>{renderGym({data:localData,catalog:localCatalog,onSelectCase:()=>{},initialMode:localMode,focusOnMount:true,figureBaseUrl:''})}</DatasetBoundary></>;}
