...
export async function readFullToolInspection(raw,view,batchId,candidateId,sessionEpoch,suffixEpoch,diagnostic){
  ...
  if(canonicalAdaptive(v)!==raw||Object.keys(v).sort().join('|')!=='diagnostic|observation|response|schema|session_epoch'||
    ...
    !['assembly_view','length_view','shadow_view','analytic_shadow_view','annular_shadow_view'].includes(diagnostic))throw Error('Full tool inspection differs from current phase/state.');
  const shadow=['shadow_view','analytic_shadow_view','annular_shadow_view'].includes(diagnostic);
  ...
  return reader(canonicalAdaptive(v.response),view.suffix,batchId,candidateId,suffixEpoch,
    shadow?(diagnostic==='annular_shadow_view'?'closed_annular_axis_point_shadow_1':diagnostic==='analytic_shadow_view'?'closed_analytic_axis_point_shadow_1':'closed_box_axis_point_shadow_1'):undefined);
}
