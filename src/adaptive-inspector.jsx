...
import {TransformEnclosureDetails} from './adaptive-transform-details.jsx';
...
import {sourceFaceDisplay,sourceFaceChoices} from './cad-face-display.mjs';
import {sourceFaceContactText} from './cad-cell-faces.mjs';
import {adaptiveCellExplanation} from './adaptive-cell-explanation.mjs';
import {recordedPartitionContext,recordedTransitionContext} from './adaptive-inspection-context.mjs';
...
import {stationaryTurningShadowCellRelations} from './stationary-turning-shadow-view.mjs';
...
export function AdaptiveInspector({prepared,onClose,live=null,initialStock=false,sourceInspection=null}){
  ...
  const stationaryTurningShadow=shadowPreview?.projection.schema==='adaptive-stationary-turning-point-shadow-view-1';
  const turningShadow=stationaryTurningShadow||shadowPreview?.projection.schema==='adaptive-turning-point-shadow-view-1';
  const turningProjection=stationaryTurningShadow?shadowPreview.projection.rotating_projection:shadowPreview?.projection;
  const shadowCells=useMemo(()=>shadowPreview?(stationaryTurningShadow?stationaryTurningShadowCellRelations:turningShadow?turningShadowCellRelations:shadowCellRelations)(bundle,bundle.frames[index],shadowPreview):null,[bundle,index,shadowPreview,turningShadow,stationaryTurningShadow]);
  ...
  const faceInventory=useMemo(()=>{
    try{return {faces:sourceFaceChoices(bundle.source.target_construction),error:''};}
    catch(error){return {faces:[],error:error.message};}
  },[bundle.source]);
  const effectivePickMode=faceInventory.faces.length?pickMode:'cell';
  ...
  const partitionContext=useMemo(()=>recordedPartitionContext(frame),[frame]);
  const transitionContext=useMemo(()=>recordedTransitionContext(bundle,index),[bundle,index]);
  ...
  const faceOverlay=useMemo(()=>{
    if(effectivePickMode==='face'){
      try{return {meshes:sourceFaceDisplay(bundle.source.target_construction,
        faceInventory.faces.map(f=>f.session_index)),error:''};}
  ...
  },[sourceFaces,highlightFaces,faceChoice,bundle.source,effectivePickMode,faceInventory]);
  ...
  const currentEvidence=loadedEvidence&&loadedEvidence.selected===selected&&loadedEvidence.material===frame.state_hash&&loadedEvidence.source===bundle.source&&loadedEvidence.context===faceInspection?.sourceFaceContext?loadedEvidence:null;
  const leaf=selected===null?null:frame.domain.leaves[selected],certificate=leaf?(bundle.certificate_mode==='on_demand'?currentEvidence?.certificate:bundle.certificates[frame.certificate_refs[selected]]?.certificate):null;
  ...
  async function loadEvidence(){
    ...
    try{const c=await live.loadCellEvidence(selected);if(token===evidenceToken.current)setLoadedEvidence({certificate:c.certificate??c,transformEnclosures:c.transformEnclosures??null,selected,material:frame.state_hash,source:bundle.source,context:faceInspection?.sourceFaceContext});}
  ...
  function exportInspection(){
    ...
    try{
      ...
      const lines=[
        ...
        `Partition: ${partitionContext.cell_count} cells · depth ${partitionContext.min_depth}–${partitionContext.max_depth} · ${partitionContext.stop_label} (producer metadata)`,
        `Transition: ${transitionContext?`${transitionContext.status} · new credited removal ${interval(transitionContext.credited_removal)} mm³${transitionContext.duplicate_delivery?' · duplicate delivery; no new credit':''}`:'no result record supplied'}`,
  ...
  return <section id="gym" className="adaptive-inspector" aria-label="Adaptive episode inspector">
    ...
    <details className="adaptive-recorded-context"><summary>Partition and recorded transition</summary>
      <section aria-label="Recorded partition context"><h3>Partition</h3>
        <p>{partitionContext.cell_count.toLocaleString('en-US')} cells · depth {partitionContext.min_depth}–{partitionContext.max_depth}</p>
        <p>{partitionContext.stop_label} · <code>{partitionContext.stop_reason}</code></p>
        <p className="adaptive-small">Producer-recorded partition status. Later refinement can retain an earlier build status; this is not a recorded cause for each cell.</p>
      </section>
      <section aria-label="Recorded transition comparison"><h3>Recorded transition</h3>
        {transitionContext?<><p>{transitionContext.status} · {transitionContext.reason.replaceAll('_',' ')}{transitionContext.duplicate_delivery?' · duplicate delivery; no new credit':''}</p>
          <dl><div><dt>Before remaining delta (mm³)</dt><dd>{transitionContext.before_remaining?interval(transitionContext.before_remaining):'Before frame not supplied'}</dd></div>
            <div><dt>After remaining delta (mm³)</dt><dd>{interval(transitionContext.after_remaining)}</dd></div>
            <div><dt>New credited removal (mm³)</dt><dd>{interval(transitionContext.credited_removal)}</dd></div>
            {transitionContext.original_removal&&<div><dt>Original result removal (mm³)</dt><dd>{interval(transitionContext.original_removal)}</dd></div>}</dl>
          <p className="adaptive-small">Bounds use their recorded partitions. Credited removal is a separate producer record; it is not obtained by subtracting displayed bounds.</p>
          <p className="adaptive-small">Before <code>{transitionContext.before_hash}</code><br/>After <code>{transitionContext.after_hash}</code><br/>Event <code>{transitionContext.event_id??'No accepted event'}</code></p>
          <p className="adaptive-small">Record bindings are checked. This display does not perform geometric replay.</p>
        </>:<p>No transition result supplied for this snapshot.</p>}
      </section>
    </details>
    ...
    {faceInventory.error&&<p role="alert">{faceInventory.error}</p>}
    ...
    {shadowPreview&&<div aria-label="Directional shadow inspection">
      <p className="adaptive-small">{turningShadow?<>Turning about spindle {'XYZ'[turningProjection.spindle.axis]} · {turningProjection.mode==='OUTSIDE'?'inward radial approach':`facing from the ${turningProjection.facing_sign>0?'negative':'positive'} end`}. {stationaryTurningShadow?<>Stationary point shadow assessed at fixed bearing {'XYZ'[shadowPreview.projection.radial_axis]}{shadowPreview.projection.radial_sign>0?'+':'−'}. Finite-tool access and full-rotation machine collision are not assessed.</>:<>Complete rotation; stationary obstacles and finite-tool access are not assessed.</>}</>:<>Direction {'XYZ'[shadowPreview.projection.axis]}{shadowPreview.projection.sign>0?'+':'−'} in part coordinates. {shadowPreview.obstacle_context.pose_is_current?'Candidate uses the accepted index.':'Candidate uses a proposed index; the workpiece has not moved.'}</>} Purple is the 3D shadow surface. The section uses conservative cells.</p>
      {shadowDisplayError?<p role="alert">Shadow surface unavailable: {shadowDisplayError}</p>:currentShadow?<p className="adaptive-small" data-shadow-display={currentShadow.projection_id}>Shadow layer ready · {currentShadow.meshes.shadow.indices.length?'display approximation':currentShadow.stationary_axis_contacts?'no volumetric material shadow; closed boundary contact retained':'empty material shadow at this candidate'} · {shadowCells.filter(s=>s==='inside').length} definite cells · {shadowCells.filter(s=>s==='mixed_or_unresolved').length} unresolved cells.</p>:<p role="status">Building shadow surface…</p>}
      {!!currentShadow?.stationary_axis_contacts&&<p className="adaptive-small" data-shadow-axis-contact={currentShadow.stationary_axis_contacts}>The stationary projection contains {currentShadow.stationary_axis_contacts} closed spindle-axis contact line(s). These have zero volume and are not drawn as solid surfaces. Conservative section cells retain boundary uncertainty.</p>}
    </div>}
    ...
    <div className="adaptive-evidence"><section><h3>Cell evidence</h3><label>Select a cell<select aria-label="Adaptive cell" value={selected??''} onChange={e=>selectCell(e.target.value===''?null:+e.target.value)}><option value="">Pick a cell in either view</option>{frame.domain.leaves.map((c,i)=><option key={i} value={i}>Depth {c.address.depth} · Morton {c.address.morton_prefix} · {category(c,frame.coverage[i])||'no delta'}</option>)}</select></label>{leaf&&<><dl><dt>Bounds (mm)</dt><dd>{selectedBounds.map(v=>v.map(fmt).join(', ')).join(' → ')}</dd><dt>Source relations</dt><dd>Stock: {leaf.stock}; target: {leaf.target}; protected: {leaf.protected}</dd><dt>Removal coverage</dt><dd>Definite: {String(frame.coverage[selected][0])}; possible: {String(frame.coverage[selected][1])}</dd></dl>{certificate?<details><summary>Exact predicate certificate</summary><pre>{canonicalAdaptive(certificate,true)}</pre></details>:<p>Predicate evidence has not been loaded for this cell.</p>}{currentEvidence?.transformEnclosures&&<TransformEnclosureDetails record={currentEvidence.transformEnclosures}/>}{bundle.certificate_mode==='on_demand'&&live?.loadCellEvidence&&<button disabled={evidenceBusy||sourceFacesBusy||live.busy} onClick={loadEvidence}>{evidenceBusy?'Loading evidence…':'Load predicate certificate'}</button>}{construction&&faceInspection?.loadSourceFaces&&<div aria-label="Cell source faces"><button disabled={evidenceBusy||sourceFacesBusy||live?.busy} onClick={loadFaces}>{sourceFacesBusy?'Loading source faces…':'Load original CAD faces'}</button>{sourceFacesBusy&&faceInspection.cancelSourceFaces&&<button onClick={faceInspection.cancelSourceFaces}>Cancel face query</button>}{sourceFaces&&<><p>{sourceFaceContactText(sourceFaces)}</p><p className="adaptive-small">{sourceFaces.schema==='adaptive-cad-cell-faces-2'?'Rational nominal faces: unresolved candidates are not proven contacts. Highlights approximate the nominal faces and do not establish contact or tool access.':<>Closed-cell contact in the {spherical?'spherical nominal':periodic?'periodic nominal':'imported nominal'} model, including edges and tangencies. This does not establish tool access.</>}</p>{sourceFaces.faces.length>0&&<div className="view-controls"><label><input type="checkbox" aria-label="Highlight source faces" checked={highlightFaces} onChange={e=>setHighlightFaces(e.target.checked)}/>Show source faces through stock</label><label>Highlight<select aria-label="Highlighted CAD face" value={faceChoice} onChange={e=>setFaceChoice(e.target.value)}><option value="all">{rational?'All associated faces':'All touching faces'}</option>{sourceFaces.faces.map(f=><option key={f.source_face_id} value={String(f.source_face_index)}>Face {f.source_face_index}{f.relation==='UNRESOLVED'?' (candidate)':''}</option>)}</select></label><span className="adaptive-small">Orange: original nominal faces. Curved highlights use a display mesh.</span></div>}{faceOverlay.error&&<p role="alert">{faceOverlay.error}</p>}<details><summary>Source face identities</summary><pre>{canonicalAdaptive(sourceFaces,true)}</pre></details><CadFaceProvenancePanel certificate={bundle.source.target_construction} attachment={faceInspection?.provenanceAttachment} associations={sourceFaces}/></>}</div>}{evidenceError&&<p role="alert">{evidenceError}</p>}</>}</section>
