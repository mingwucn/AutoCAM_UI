...
const colors={target:'#afcbd9',definite:'#004070',uncertain:'#e5ac48',removed:'#3aa99d',remove:'#f5a544',reach:'#df654c',cuttingLength:'#b8942a',assemblyCutting:'#e5ac48',assemblyBody:'#647e8c',assemblyArbor:'#8b9da6',assemblyHolder:'#004070',assemblyFixed:'#c84d40'};
...
export function AdaptiveInspector({prepared,onClose,live=null,initialStock=false,sourceInspection=null}){
  ...
  const [layers,setLayers]=useState({target:true,remaining:true,holding:true,remove:true,reach:true,cuttingLength:true,assemblyCutting:true,assemblyBody:true,assemblyArbor:true,assemblyHolder:true,assemblyFixed:true,definite:!initialStock&&!solidStock,uncertain:!initialStock&&!solidStock,removed:!initialStock});
  ...
  return <section id="gym" className="adaptive-inspector" aria-label="Adaptive episode inspector">
    ...
    <div className="view-controls"><button onClick={()=>view.current?.home()} disabled={!webgl}>Home view</button>{(solidStock?['remaining','target','holding',...(live?.drillLayers?['remove',...(lengthPreview?['reach','cuttingLength']:[])]:[]),'removed','definite','uncertain']:['target','definite','uncertain','removed']).map(key=><label key={key}><input type="checkbox" checked={layers[key]} onChange={e=>setLayers({...layers,[key]:e.target.checked})}/><i className="swatch" style={{background:colors[key]??(key==='remaining'?'#004070':'#cb7d36')}}/>{({remaining:'Remaining stock',target:'Target',holding:'Holding',definite:solidStock?'Certain remaining cells':'Certain remaining',uncertain:solidStock?'Uncertain cells':'Uncertain',removed:'Removed',remove:'Remove',reach:'Beyond reach',cuttingLength:'Beyond cutting length'})[key]}</label>)}</div>
