...
(async()=>{
  ...
  const vectors=JSON.parse(raw),modulePath=path.resolve(__dirname,'../src/adaptive-mill-turn-model.mjs');
  ...
  await consumer.loadMillTurnCheckpoint(new Uint8Array(vectors.checkpoint_bytes),vectors.checkpoint_sha256);
  for(const vector of vectors.cases){
    let result;
    try{const value=consumer.scoreMillTurnCandidate(vector.checkpoint,vector.features,vector.mask);result={action:value.action,score_bits:value.scores.map(scoreBits)};}
  ...
  try{
    ...
    const results=await page.evaluate(async data=>{
      ...
      await model.loadMillTurnCheckpoint(new Uint8Array(data.checkpoint_bytes),data.checkpoint_sha256);
      ...
      return data.cases.map(vector=>{
        try{const value=model.scoreMillTurnCandidate(vector.checkpoint,vector.features,vector.mask);return {action:value.action,score_bits:value.scores.map(bits)};}
