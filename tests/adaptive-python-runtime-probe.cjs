...
const workerSource=fs.readFileSync(path.join(__dirname,'adaptive-python-probe-worker.mjs'),'utf8');

(async()=>{
  const directory=path.resolve(process.argv[2]);
  fs.writeFileSync(path.join(directory,'probe-worker.mjs'),workerSource);
  fs.writeFileSync(path.join(directory,'index.html'),'<!doctype html><meta charset="utf-8"><title>Shared Python runtime probe</title><p>Local worker compatibility check</p>');
