...
const result=update.apply(this,args),options=args[2];
(window.faceFrameAudit??=[]).push({selected:options.selected,faces:(options.sourceFaceMeshes??[]).map(f=>f.sourceFaceIndex)});
...
(async()=>{
  ...
  try{
    ...
    assert(frameAudit.filter(r=>r.selected===0||r.selected===null).every(r=>r.faces.length===0));
