...
(async()=>{
  ...
  const vectors=JSON.parse(raw),modulePath=path.resolve(__dirname,'../src/adaptive-bounded-mill-turn-model.mjs');
  ...
  await consumer.loadBoundedMillTurnCheckpoint(new Uint8Array(vectors.checkpoint_bytes),vectors.checkpoint_sha256);
  for(const vector of vectors.cases){
    let result;
    try{const value=consumer.scoreBoundedMillTurnCandidate(vector.checkpoint,vector.features,vector.mask);result={action:value.action,score_bits:value.scores.map(scoreBits)};}
    catch(error){result={error:true};}
    check(result,vector);
  }
  const built=esbuild.buildSync({entryPoints:[modulePath],bundle:true,format:'iife',globalName:'AdaptiveMixedConsumer',write:false,target:'es2020'}).outputFiles[0].text;
  fs.writeFileSync(path.join(directory,'browser-consumer.js'),built);
  const browser=await chromium.launch({channel:'chrome',headless:true});
  try{
    const page=await browser.newPage();
    await page.route('**/*',route=>route.fulfill({status:200,contentType:'text/html',body:'<!doctype html><title>Mixed model numeric parity</title>'}));
    ...
    const results=await page.evaluate(async data=>{
      const model=AdaptiveMixedConsumer;
      await model.loadBoundedMillTurnCheckpoint(new Uint8Array(data.checkpoint_bytes),data.checkpoint_sha256);
      ...
      return data.cases.map(vector=>{
        try{const value=model.scoreBoundedMillTurnCandidate(vector.checkpoint,vector.features,vector.mask);return {action:value.action,score_bits:value.scores.map(bits)};}
