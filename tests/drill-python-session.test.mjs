...
test('drill owner retains explicit commands and epoch checks across worker recovery',async()=>{
  const clients=[];
  const owner=new DrillPythonSession('worker',{clientFactory:(_url,options)=>{
  ...
  await owner.invoke('{"operation":"view"}');assert.equal(owner.journal.length,7);
...
test('drill model load refuses before sending a worker command',async()=>{
  const owner=new DrillPythonSession('worker',{clientFactory:()=>{throw Error('must not create worker');}});
