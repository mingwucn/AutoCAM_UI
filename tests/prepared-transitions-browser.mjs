...
const output=process.argv.includes('--output')?path.resolve(option('--output')):await fs.mkdtemp(path.join(root,'test-results/prepared-transitions-'));
...
const fixtures=path.join(root,'tests/fixtures/prepared-transitions'),manifest=JSON.parse(await fs.readFile(path.join(fixtures,'manifest.json'))),inputs={};
assert.equal(manifest.schema,'autocam-prepared-transition-fixtures-1');
...
try{
 ...
 await context.addInitScript(()=>{
  ...
  window.Worker=class extends NativeWorker{
   set onmessage(handler){super.onmessage=event=>{
    ...
    if(event.data?.type==='result'&&typeof raw==='string'&&raw.includes('"schema":"adaptive-prepared-browser-transition-record-1"')){
 ...
 for(const name of ['drill','drill-prefix','face','face-prefix']){
  if(name!=='drill')await panel.getByRole('button',{name:'Close live case',exact:true}).click();
  const family=name.startsWith('drill')?'drill':'face';
  ...
  const initial=inputs[name+'/initial-transitions.json'],expected=inputs[name+'/selected-transitions.json'];
  assert.deepEqual(await save('Download transitions',name+'-initial.json'),initial);
  assert.deepEqual(await save('Download decisions',name+'-initial-episode.json'),inputs[name+'/initial-episode.json']);
  const restore=panel.getByLabel('Restore '+family+' decisions',{exact:true});
  await restore.setInputFiles({name:'decisions.json',mimeType:'application/json',buffer:inputs[name+'/episode.json']});await ready();
  ...
  assert.deepEqual(episode,inputs[name+'/episode.json']);assert.deepEqual(transitions,expected);
  ...
  assert.deepEqual(await save('Download transitions',name+'-reset.json'),initial);
  await panel.getByRole('button',{name:'Prepare '+family+' candidates',exact:true}).click();await ready();
  const generated=JSON.parse(await save('Download transitions',name+'-generated.json'));
  assert.deepEqual(generated.material.records,JSON.parse(initial).material.records);
  assert.equal(generated.material.final_state_hash,JSON.parse(initial).material.final_state_hash);
  if(!name.endsWith('-prefix')){
   await panel.getByRole('button',{name:'Preview saved candidate',exact:true}).click();await ready();
   assert.deepEqual(JSON.parse(await save('Download transitions',name+'-preview.json')),generated);
   await panel.getByRole('button',{name:family==='drill'?'Execute prepared drill':'Execute prepared face mill',exact:true}).click();await ready();
   const selected=JSON.parse(await save('Download transitions',name+'-interactive.json'));
   assert.deepEqual(selected.material.records,JSON.parse(expected).material.records);
   assert.deepEqual(selected.material.snapshots,JSON.parse(expected).material.snapshots);
   assert.equal(selected.material.final_state_hash,JSON.parse(expected).material.final_state_hash);
   const decision=await save('Download decisions',name+'-interactive-episode.json');
   assert.equal(selected.material.episode.sha256,sha(decision));
  }
  await restore.setInputFiles({name:'decisions.json',mimeType:'application/json',buffer:episode});await ready();
  assert.deepEqual(await save('Download transitions',name+'-restored.json'),transitions);
  checks.push({name,local_files:true,native_downloads_exact:true,inherited_prefix:name.endsWith('-prefix'),interactive_selection:!name.endsWith('-prefix'),generation_read_only:true,reset_exact:true,restore_exact:true});
  console.log('Prepared full-site passed: '+name);
 }
 ...
 assert.equal((await save('Download transitions','after-corruption-transitions.json')).toString(),inputs['face-prefix/selected-transitions.json'].toString());
 ...
 for(const family of ['mixed','full']){
  await panel.getByRole('button',{name:'Close live case',exact:true}).click();
  const base=path.join(root,'tests/fixtures/mixed-tool-inspection',family);
  await loader.getByLabel('Task JSON',{exact:true}).setInputFiles(path.join(base,'task.json'));
 ...
 console.log('Production prepared transition checks passed: '+output);
